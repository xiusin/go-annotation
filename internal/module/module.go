...
import (
	...
	"github.com/xiusin/go-annotation/internal/environment"
...
// Usecase:
// m.root: /home/yury/go/src/github.com/xiusin/go-annotation/examples/constructor
// m.files: /internal/common/common.go, ...
// importPath: github.com/xiusin/go-annotation/examples/constructor/internal/common
...
func (m *module) escapedPath(moduleName string) (string, bool) {
	if m.mod == nil || len(m.mod.Require) == 0 {
		log.Println("escapedPath: mod file nil or no required")
		return "", false
	}
	line := findModuleLine(m.mod.Require, moduleName)
	if line == nil {
		log.Println("escapedPath: no module line")
		return "", false
	}

	if len(line.Token) < 2 {
		log.Println("escapedPath: invalid line token")
		return "", false
	}
	p, err := escapedPath(line)
	if err != nil {
		log.Fatalf("escapedPath: exc path err on %s: %s", line.Token[0], err.Error())
		return "", false
	}
	v, err := escapedVersion(line)
	if err != nil {
		log.Fatalf("escapedPath: exc version err: %s", err.Error())
		return "", false
	}
	return p + "@" + v, true
}
